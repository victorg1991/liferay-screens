...
open class CookieCredentialsStoreKeyChain : BaseCredentialsStoreKeyChain {
	...
	override open func storeAuth(keychain: Keychain, auth: LRAuthentication) {
		...
		do {
			...
			try keychain.set(cookieAuth.username, key: "basicauth_username")
			try keychain.set(cookieAuth.password, key: "basicauth_password")
		}
		catch {
		}
	}
	...
	override open func loadAuth(keychain: Keychain) -> LRAuthentication? {
		let username = try? keychain.get("cookie_username")
		let password = try? keychain.get("cookie_password")

		if let username = username,
			let password = password {

			return LRCookieAuthentication(authToken: "", cookieHeader: "",
				username: username,
				password: password)
		}
